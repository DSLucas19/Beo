...
export default function App() {
  ...
  const getWizardCardClass = (stepIndex) => {
    if (stepIndex === wizardStep) return "wizard-card-active";
    if (stepIndex > wizardStep) return "wizard-card-future";
    
    const diff = wizardStep - stepIndex;
    if (diff === 1) {
      return stepIndex === 1 ? "wizard-card-past-1-near" : "wizard-card-past-2-near";
    }
    if (diff === 2) {
      return "wizard-card-past-1-far";
    }
    return "wizard-card-past-deep";
  }

  const handleStep3KeyDown = (e) => {
    if (e.key === 'Enter' && apiKey.trim() && !testingConnection && !scanningModels) {
      e.preventDefault();
      handleLaunch();
    }
  }
  ...
  const needsGatewayUrl = aiProvider === 'cliproxyapi' || aiProvider === 'openrouter' || aiProvider === 'custom' || aiProvider === 'mimo'
  ...
  const getDefaultGateway = () => {
    ...
    if (aiProvider === 'mimo') return 'https://api.xiaomimimo.com/v1'
  ...
  const getKeyPlaceholder = () => {
    ...
    if (aiProvider === 'mimo') return 'MiMo API Key (mimo-key-...)'
  ...
  const getKeyLabel = () => {
    ...
    if (aiProvider === 'mimo') return 'MiMo API Key'
  ...
  const handleLaunch = async () => {
    ...
    } else if (aiProvider === 'mimo') {
      urlParam = gatewayUrl.trim() || 'https://api.xiaomimimo.com/v1'
    } else if (aiProvider === 'custom') {
      urlParam = customEndpointUrl.trim() || null
    }
...
Hãy giúp tôi xây dựng và phác thảo các tài liệu cấu trúc cốt lõi của doanh nghiệp (AIM.md, OPERATIONS.md và FINANCE.md) để bắt đầu vận hành công ty ngay lập tức!`
    ...
    // Trigger welcoming prompt to Secretary AI in the background (no await)
    // so the user sees the message and the AI response stream in real-time on the dashboard
...
Hãy mở PowerShell / Command Prompt trên máy của bạn, dán lệnh trên và chạy để xác thực!`)
    ...
    return (
      <button
        ...
        onClick={() => {
          ...
          if (p.id === 'openrouter') {
            setGatewayUrl('https://openrouter.ai/api/v1')
            setCustomModel('')
          } else if (p.id === 'cliproxyapi') {
            setGatewayUrl('http://localhost:8317/v1')
            setCustomModel('')
          } else if (p.id === 'mimo') {
            setGatewayUrl('https://api.xiaomimimo.com/v1')
            setCustomModel('mimo-v2.5-pro')
          } else if (p.id === 'custom') {
            setGatewayUrl('')
            setCustomModel('')
          } else {
  ...
  return (
    <div className="w-screen h-screen flex topography-bg text-content-normal overflow-hidden relative">
      ...
      {showSetupWizard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-10 transition-all duration-500 overflow-hidden">
          <div className="wizard-card-container w-full max-w-[460px]">

            {/* STEP 1: Company Name */}
            <div className={`wizard-card ${getWizardCardClass(1)}`}>
              <button
            ...
            <div className={`wizard-card ${getWizardCardClass(2)}`}>
              <button
                onClick={() => setSetupWizard(false)}
                className="absolute bottom-3 left-3 z-20 px-3 py-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] text-[11px] text-zinc-400 hover:text-white border border-white/[0.06]"
              >
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <textarea
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' && !e.shiftKey && companyAim.trim()) {
                            e.preventDefault()
                            setWizardStep(3)
                          }
                        }}
            ...
            <div className={`wizard-card ${getWizardCardClass(3)}`}>
              <button
                onClick={() => setSetupWizard(false)}
                className="absolute bottom-3 left-3 z-20 px-3 py-1.5 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] text-[11px] text-zinc-400 hover:text-white border border-white/[0.06]"
              >
              ...
              <div className="flex-1 overflow-y-auto p-6 pb-12">
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-4 border-t border-white/[0.04] pt-4 animate-fade-in">
                    ...
                    {/* Gateway URL — for CLIProxy, OpenRouter, MiMo */}
                    {(aiProvider === 'cliproxyapi' || aiProvider === 'openrouter' || aiProvider === 'mimo') && (
                      <div className="space-y-1.5 pane-fade-in">
                        <label className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">
                          {aiProvider === 'openrouter' ? 'OpenRouter Gateway' : aiProvider === 'mimo' ? 'MiMo API Base URL' : 'CLIProxyAPI Gateway URL'}
                        </label>
                        <div className="relative">
                          <GlobeIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                          <input
                    ...
                    {/* Model Name Input — visible for Custom, OpenRouter, CLIProxyAPI, MiMo */}
                    {(aiProvider === 'custom' || aiProvider === 'openrouter' || aiProvider === 'cliproxyapi' || aiProvider === 'mimo') && (
            ...
            <div className={`wizard-card ${getWizardCardClass(4)}`}>
              <div className="absolute top-0 left-0 right-0 h-1.5 lava-bg opacity-80 z-10" />
              <div className="flex-1 overflow-y-auto p-6 pb-12 flex flex-col justify-center select-none">
